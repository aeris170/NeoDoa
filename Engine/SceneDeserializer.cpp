...
static const tinyxml2::XMLElement* findComponentByNameIn(const tinyxml2::XMLElement* parent, std::string_view name) {
    for (const tinyxml2::XMLElement* comp = parent->FirstChildElement(); comp != nullptr; comp = comp->NextSiblingElement()) {
...
static void loopElement(const tinyxml2::XMLElement* element, auto lambda) {
    for (const tinyxml2::XMLElement* l = element; l != nullptr; l = l->NextSiblingElement()) {
        lambda(l);
    }
}
...
Scene DeserializeScene(const std::string& data) {
    ...
    { // fill config
        ...
        {
            const tinyxml2::XMLElement* ortho = camerasNode->FirstChildElement("orthoCamera");
        ...
        {
            const tinyxml2::XMLElement* perpective = camerasNode->FirstChildElement("perspectiveCamera");
...
Entity DeserializeEntityID(const tinyxml2::XMLElement* property, std::string_view identifierOverride) {
...
ParentComponent DeserializeParentComponent(const tinyxml2::XMLElement* component, const Entity entity) {
    ...
    loopElement(component->FirstChildElement(nameof_c(ParentComponent::children))->FirstChildElement()->FirstChildElement(), [&children](const tinyxml2::XMLElement* l) {
