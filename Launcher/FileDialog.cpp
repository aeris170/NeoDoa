...
bool FolderNode(const char* label, ImTextureID icon, bool& clicked)
{
	...
	float text_posX = icon_posX + g.Style.FramePadding.y + IFD_ICON_SIZE;
	...
	window->DrawList->AddImage(icon, ImVec2(icon_posX, pos.y), ImVec2(icon_posX + IFD_ICON_SIZE, pos.y + IFD_ICON_SIZE));
...
bool FileNode(const char* label, ImTextureID icon) {
	...
	// Icon, text
	window->DrawList->AddImage(icon, ImVec2(pos.x, pos.y), ImVec2(pos.x + IFD_ICON_SIZE, pos.y + IFD_ICON_SIZE));
	ImGui::RenderText(ImVec2(pos.x + g.Style.FramePadding.y + IFD_ICON_SIZE, pos.y + g.Style.FramePadding.y), label);
...
bool PathBox(const char* label, std::filesystem::path& path, char* pathBuffer, ImVec2 size_arg) {
	...
	ImVec2 size = ImGui::CalcItemSize(size_arg, 200, IFD_GUI_ELEMENT_SIZE);
	...
	if (!(*state & 0b001)) {
		...
		for (auto comp : path) {
			...
			totalWidth += ImGui::CalcTextSize(section.c_str()).x + style.FramePadding.x * 2.0f + IFD_GUI_ELEMENT_SIZE;
			btnList.push_back(section);
		}
		totalWidth -= IFD_GUI_ELEMENT_SIZE;
		...
		for (size_t i = 0; i < btnList.size(); i++) {
			if (totalWidth > size.x - 30 && i != btnList.size() - 1) { // trim some buttons if there's not enough space
				float elSize = ImGui::CalcTextSize(btnList[i].c_str()).x + style.FramePadding.x * 2.0f + IFD_GUI_ELEMENT_SIZE;
				totalWidth -= elSize;
				continue;
			}
			...
			if (!isFirstElement) {
				ImGui::ArrowButtonEx("##dir_dropdown", ImGuiDir_Right, ImVec2(IFD_GUI_ELEMENT_SIZE, IFD_GUI_ELEMENT_SIZE));
				anyOtherHC |= ImGui::IsItemHovered() | ImGui::IsItemClicked();
				ImGui::SameLine();
			}
			if (ImGui::Button(btnList[i].c_str(), ImVec2(0, IFD_GUI_ELEMENT_SIZE))) {
#ifdef _WIN32
				std::string newPath = "";
#else
				std::string newPath = "/";
...
bool FavoriteButton(const char* label, bool isFavorite)
{
	...
	ImVec2 pos = window->DC.CursorPos;
	bool ret = ImGui::InvisibleButton(label, ImVec2(IFD_GUI_ELEMENT_SIZE, IFD_GUI_ELEMENT_SIZE));
...
FileDialog::FileData::FileData(const std::filesystem::path& path) {
	...
	struct stat attr;
#ifdef _WIN32
	_wstat64i32(path.wstring().c_str(), (struct _stat64i32*)&attr);
#else
...
bool FileDialog::IsDone(const std::string& key)
{
	...
	if (isMe && m_isOpen) {
		if (!m_calledOpenPopup) {
			static auto size = ImVec2(ImGui::GetIO().DisplaySize.x, ImGui::GetIO().DisplaySize.y);
			ImGui::SetNextWindowSize(size, ImGuiCond_FirstUseEver);
			ImGui::OpenPopup(m_currentTitle.c_str());
			m_calledOpenPopup = true;
		}

		if (ImGui::BeginPopupModal(m_currentTitle.c_str(), &m_isOpen, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoScrollbar)) {
			m_renderFileDialog();
			ImGui::EndPopup();
		}
		else m_isOpen = false;
...
void* FileDialog::m_getIcon(const std::filesystem::path& path)
{
	...
	// dark theme - invert the colors
		...
		for (int y = 0; y < 32; y++) {
			for (int x = 0; x < 32; x++) {
				int index = (y* IFD_DEFAULT_ICON_SIZE + x) * 4;
		...
		m_icons[pathU8] = this->CreateTexture(invData, IFD_DEFAULT_ICON_SIZE, IFD_DEFAULT_ICON_SIZE, 0);
	...
	if (m_zoom == 1.0f) {
		if (ImGui::BeginTable("##contentTable", 3, /*ImGuiTableFlags_Resizable |*/ ImGuiTableFlags_Sortable, ImVec2(0, -FLT_MIN))) {
			...
			for (auto& entry : m_content) {
				...
				ImGui::Image((ImTextureID)m_getIcon(entry.Path), ImVec2(IFD_ICON_SIZE, IFD_ICON_SIZE));
				ImGui::SameLine();
...
void FileDialog::m_renderFileDialog()
{
	...
	if (ImGui::ArrowButtonEx("##back", ImGuiDir_Left, ImVec2(IFD_GUI_ELEMENT_SIZE, IFD_GUI_ELEMENT_SIZE), m_backHistory.empty() * ImGuiItemFlags_Disabled)) {
	...
	if (ImGui::ArrowButtonEx("##forward", ImGuiDir_Right, ImVec2(IFD_GUI_ELEMENT_SIZE, IFD_GUI_ELEMENT_SIZE), m_forwardHistory.empty() * ImGuiItemFlags_Disabled)) {
	...
	if (ImGui::ArrowButtonEx("##up", ImGuiDir_Up, ImVec2(IFD_GUI_ELEMENT_SIZE, IFD_GUI_ELEMENT_SIZE))) {
	...
	if (PathBox("##pathbox", curDirCopy, m_pathBuffer, ImVec2(-250, IFD_GUI_ELEMENT_SIZE)))
	...
	if (ImGui::InputTextEx("##searchTB", "Search", m_searchBuffer, 128, ImVec2(-FLT_MIN, IFD_GUI_ELEMENT_SIZE), 0)) // TODO: no hardcoded literals
		m_setDirectory(m_currentDirectory, false); // refresh
	...
	// buttons
	float ok_cancel_width = IFD_GUI_ELEMENT_SIZE * 7;
