...
ComponentInstance* UserDefinedComponentStorage::AttachComponent(UUID component, Assets& assets) {
    ...
    if (!handle.HasValue()) {
        ...
        return &components.try_emplace(component.AsString(), component, assets, InstantiationError::DEFINITION_MISSING).first->second;
    }
    if (!handle->IsComponentDefinition()) {
        DOA_LOG_ERROR("Something went wrong! Tried to instantiate a non-component!");
        return &components.try_emplace(handle->File().Name().data(), component, assets, InstantiationError::NON_DEFITION_INSTANTIATION).first->second;
    }
    if (!handle->HasDeserializedData()) {
        DOA_LOG_ERROR("Something went wrong! Tried to instantiate a non-deserialized component!");
        return &components.try_emplace(handle->File().Name().data(), component, assets, InstantiationError::DEFINITION_NOT_DESERIALIZED).first->second;
    }
